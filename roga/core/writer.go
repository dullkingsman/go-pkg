...
func (d DefaultWriter) WriteOperationsToStdout(items []Operation, r *Roga) {
	for _, item := range items {
		...
		if item.Measurements != nil {
			...
			if err != nil {
				utils.LogError("handler-error", "could not marshal additional values")
			} else {
...
func (d DefaultWriter) WriteOperationsToFile(items []Operation, r *Roga) {
	// TODO implementation
}
...
func (d DefaultWriter) WriteLogsToFile(items []Log, r *Roga) {
	// TODO implementation
}
